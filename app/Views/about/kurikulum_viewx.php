<?= $this->extend('layout/template-page'); ?>
<?= $this->section('content'); ?>
  <!-- ======= Breadcrumbs ======= -->
  <?= $this->include('layout/breadcrumbsx'); ?>
  <!-- End Breadcrumbs -->
    <div class="main">
        <section class="module">
          <div class="container">
            <h4>Sebaran Mata Kuliah Teknik Informatika</h4>
          <hr class="divider-w pt-10">
            <div class="row">
                <div class="col-sm-7 col-md-6">
                    <div class="panel-group" id="accordion">
                        <!-- semester1 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester1">Semester 1</a></h4>
                            </div>
                            <div class="panel-collapse collapse in" id="semester1">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1101
                                                    </td>
                                                    <td>
                                                        Islam dan Ke-NU-an 1
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1103
                                                    </td>
                                                    <td>
                                                        Bahasa Indonesia 1
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1105
                                                    </td>
                                                    <td>
                                                        Bahasa Inggris 1
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1215
                                                    </td>
                                                    <td>
                                                        Pendidikan Pancasila
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1216
                                                    </td>
                                                    <td>
                                                        Pendidikan Kewarganegaraan
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1101
                                                    </td>
                                                    <td>
                                                        Sistem Basis Data
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        7
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1102
                                                    </td>
                                                    <td>
                                                        Dasar-dasar Pemrograman
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        8
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1104
                                                    </td>
                                                    <td>
                                                        Kalkulus
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                
                                                <tr>
                                                    <td>
                                                        9
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1105
                                                    </td>
                                                    <td>
                                                        Pengantar Sistem Digital
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester2 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester2">Semester 2</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester2">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1211
                                                    </td>
                                                    <td>
                                                        Islam dan Ke-NU-an 2
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1212	
                                                    </td>
                                                    <td>
                                                        Filsafat Ilmu
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1213
                                                    </td>
                                                    <td>
                                                        Bahasa Indonesia 2
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        ME1214
                                                    </td>
                                                    <td>
                                                        Digital Marketing
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2313
                                                    </td>
                                                    <td>
                                                        Interaksi Manusia dan Komputer
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1207
                                                    </td>
                                                    <td>
                                                        Pengantar Organisasi Komputer
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        7
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1208
                                                    </td>
                                                    <td>
                                                        Aljabar Linear
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        8
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1209
                                                    </td>
                                                    <td>
                                                        Matematika Diskrit
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester3 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester3">Semester 3</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester3">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI1110
                                                    </td>
                                                    <td>
                                                        Kewirausahaan
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI2356
                                                    </td>
                                                    <td>
                                                        Statistika dan Probabilitas
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2310
                                                    </td>
                                                    <td>
                                                        Pemrograman Berorientasi Objek
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2311
                                                    </td>
                                                    <td>
                                                        Sistem Operasi
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2312
                                                    </td>
                                                    <td>
                                                        Komputer dan Masyarakat
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1103
                                                    </td>
                                                    <td>
                                                        Struktur Data dan Algoritma
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        7
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2417
                                                    </td>
                                                    <td>
                                                        Kecerdasan Buatan
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        8
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2315
                                                    </td>
                                                    <td>
                                                        Jaringan Komputer
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester4 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester4">Semester 4</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester4">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        ME2415
                                                    </td>
                                                    <td>
                                                        E-Business
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2416
                                                    </td>
                                                    <td>
                                                        Sistem Multimedia
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2314
                                                    </td>
                                                    <td>
                                                        Grafika Komputer
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2418
                                                    </td>
                                                    <td>
                                                        Machine Learning
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2419
                                                    </td>
                                                    <td>
                                                        Metode Numerik
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2420
                                                    </td>
                                                    <td>
                                                        Cloud Computing
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        7
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI1206
                                                    </td>
                                                    <td>
                                                        Rekayasa Perangkat Lunak
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        8
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI2421
                                                    </td>
                                                    <td>
                                                        Pemrograman Web
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester5 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester5">Semester 5</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester5">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        ME3521
                                                    </td>
                                                    <td>
                                                        Ekonomi Teknik
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3522
                                                    </td>
                                                    <td>
                                                        Keamanan Informasi
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3523
                                                    </td>
                                                    <td>
                                                        Sistem Pakar
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3524
                                                    </td>
                                                    <td>
                                                        Pengolahan Citra Digital
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3525
                                                    </td>
                                                    <td>
                                                        Sistem Terdistribusi
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        *
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        7
                                                    </td>
                                                    <td class="hidden-xs">
                                                        **
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester6 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester6">Semester 6</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester6">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        MI3613	
                                                    </td>
                                                    <td>
                                                        Bahasa Inggris 2
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3629
                                                    </td>
                                                    <td>
                                                        Tren Teknologi
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3626
                                                    </td>
                                                    <td>
                                                        Metodologi Penelitian dan Penulisan Ilmiah
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI3627
                                                    </td>
                                                    <td>
                                                        Kerja Praktek (Magang)
                                                    </td>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        *
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td class="hidden-xs">
                                                        **
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester7 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester7">Semester 7</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester7">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI4728
                                                    </td>
                                                    <td>
                                                        KKN
                                                    </td>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI4730
                                                    </td>
                                                    <td>
                                                        Etika Profesi
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI4859
                                                    </td>
                                                    <td>
                                                        Kelas Seminar
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        4
                                                    </td>
                                                    <td class="hidden-xs">
                                                        *
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        3
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        5
                                                    </td>
                                                    <td class="hidden-xs">
                                                        **
                                                    </td>
                                                    <td>
                                                        Mata Kuliah Peminatan
                                                    </td>
                                                    <td>
                                                        2
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                        <!-- semester8 -->
                        <div class="panel panel-default">
                            <div class="panel-heading">
                            <h4 class="panel-title font-alt"><a data-toggle="collapse" data-parent="#accordion" href="#semester8">Semester 8</a></h4>
                            </div>
                            <div class="panel-collapse collapse" id="semester8">
                                <div class="panel-body">
                                    <div class="col-sm-12">
                                        <table class="table table-striped table-border checkout-table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th class="hidden-xs">Kode</th>
                                                    <th>Mata Kuliah</th>
                                                    <th>SKS</th>
                                                    <th>Ket.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        1
                                                    </td>
                                                    <td class="hidden-xs">
                                                        TI4831
                                                    </td>
                                                    <td>
                                                        SKRIPSI
                                                    </td>
                                                    <td>
                                                        6
                                                    </td>
                                                    <td>
                                                        -
                                                    </td>
                                                </tr>
                                                
                                            </tbody>
                                        </table>
                                    </div>
                                
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- sidebar bahan kajian -->
                <div class="col-sm-5 col-md-4 col-md-offset-1 sidebar">
                    <div class="shop-Cart-totalbox">
                        <h4 class="font-alt">Bahan Kajian</h4>
                        <table class="table table-hover table-border checkout-table">
                            <thead>
                                <tr>
                                    <th class="hidden-xs">Kode</th>
                                    <th class="hidden-xs">Bahan Kajian</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>BK1 </th>
                                    <td>Algoritma dan kompleksitas</td>
                                </tr>
                                <tr>
                                    <th>BK2 </th>
                                    <td>Arsitektur dan organisasi komputer</td>
                                </tr>
                                <tr>
                                    <th>BK3 </th>
                                    <td>Ilmu Komputasi</td>
                                </tr>
                                <tr>
                                    <th>BK4 </th>
                                    <td>Struktur diskrit</td>
                                </tr>
                                <tr>
                                    <th>BK5 </th>
                                    <td>Grafik dan visualisasi</td>
                                </tr>
                                <tr>
                                    <th>BK6 </th>
                                    <td>Interaksi Manusia dan Komputer</td>
                                </tr>
                                <tr>
                                    <th>BK7 </th>
                                    <td>Penjaminan dan keamanan informasi</td>
                                </tr>
                                <tr>
                                    <th>BK8 </th>
                                    <td>Manajemen Informasi</td>
                                </tr>
                                <tr>
                                    <th>BK9 </th>
                                    <td>Sistem Cerdas</td>
                                </tr>
                                <tr>
                                    <th>BK10 </th>
                                    <td>Jaringan dan komunikasi</td>
                                </tr>
                                <tr>
                                    <th>BK11 </th>
                                    <td>Sistem Operasi</td>
                                </tr>
                                <tr>
                                    <th>BK12 </th>
                                    <td>Pengembangan berbasis platform</td>
                                </tr>
                                <tr>
                                    <th>BK13 </th>
                                    <td>Komputasi paralel terdistribusi</td>
                                </tr>
                                <tr>
                                    <th>BK14 </th>
                                    <td>Bahasa Pemrograman</td>
                                </tr>
                                <tr>
                                    <th>BK15 </th>
                                    <td>Dasar pengembangan software</td>
                                </tr>
                                <tr>
                                    <th>BK16 </th>
                                    <td>Rekayasa Perangkat Lunak</td>
                                </tr>
                                <tr>
                                    <th>BK17 </th>
                                    <td>Pembentukan karakter</td>
                                </tr>
                                <tr>
                                    <th>BK18 </th>
                                    <td>Isu sosial dan praktik profesional</td>
                                </tr>
                            </tbody>
                        </table>
                        <a href="http://unusia.ac.id" class="btn btn-lg btn-block btn-round btn-d">Download Dokumen</a>
                    </div>
                </div>
            </div>
          </div>
        </section>
    </div>

<?= $this->endSection(); ?>